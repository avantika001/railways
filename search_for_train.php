        ...
        if(!$result1 || mysqli_num_rows($result1)==0){
          //output data of each row
            ...
            while($row=mysqli_fetch_assoc($result1)){
              ...
              $sql2=""; $fare="";$seats="";
              if($type=='AC_fare'){
                $sql2=mysqli_query($db,"SELECT AC_fare,A_seat from train_status where t_no='$train'");
                $row1=mysqli_fetch_assoc($sql2);
				$fare=$row1['AC_fare'];
				$seats=$row1['A_seat'];
				
              }else {
                $sql2=mysqli_query($db,"SELECT g_fare,B_seat from train_status where t_no='$train'");
                $row1=mysqli_fetch_assoc($sql2);
				$fare=$row1['g_fare'];
				$seats=$row1['B_seat'];
              }
              echo "<br><table><tr><td>Train no</td><td>:</td><td>".$row['t_no']."</td></tr><tr><td>Train Name</td><td>:</td><td>".$row['t_name']."</td></tr><tr><td>Start Time</td><td>:</td><td>".$row['start_t']."</td></tr>";
              echo "<tr><td>Fare</td><td>:</td><td>".$fare."</td></tr><tr><td>seats</td><td>:</td><td>".$seats."</td></tr></table><br>";
